...
export class InfiniteMonthView extends React.Component {
    ...
    */
    ...
    async getUserData() {
        var u = await getUser(this.props.uid)
        console.log("A",this.props.uid,u)
    ...
    _renderPage = (info) => {
        ...
        var isFriend = this.state.user.friends.includes(getMyUid())
...
class MonthView extends React.Component {
    ...
    viewPost = (day, searchIndex) => {
        /* 
        Search all posts from that date, then pass to Modal
        */

        var posts = []
        for (var i = Math.max(0, searchIndex); i < this.state.data.length; i++) {
                                ...
                                week.map((day, dayIndex) => {
                                    if (this.state.data.length > 0) {
                                        while (currSearchId < this.state.data.length) {
                                            if (this.state.data[currSearchId].dateObj.getDate() == day) {
                                                ...
                                                if ((this.state.data[currSearchId].publicOrFriends == 'friends' && !this.props.isFriend)
                                                    && this.state.data[currSearchId].userid != getMyUid()
                                                    && this.state.data[currSearchId].publicOrFriends != 'public'
                                                ) {
                                                    currSearchId++
                                                    continue
                                                }
                                        ...
                                        return <TouchableOpacity
                                            ...
                                            onPress={() => this.viewPost(day, currSearchId - 1)}>
                                            <View style={{ flex: 1 }}>
                                                <View style={{ flex: 1, backgroundColor: '#d5d3ca' }}>
                                                    <Text style={{
